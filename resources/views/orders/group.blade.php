...
@section('content')
    ...
    @endif
    <h1 class="page-header with-description">{{ $group->title }} &raquo; <a href="{{ route(!is_array(session('return_to')) ? session('return_to') : 'members.index', is_array(session('return_to_parameters')) ? session('return_to_parameters') : []) }}">Vissza</a></h1>
